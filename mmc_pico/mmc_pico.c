...
{
	uint8_t buf;
	spi_read_blocking(SPI_PORT, dat, &buf, 1);
	return (BYTE)buf;	/* Get received byte */
}
...
{
	spi_write_blocking(SPI_PORT, buff, cnt);
}
...
{
	spi_read_blocking(SPI_PORT, 0xff, buff, cnt);
}
...
static
int select_ (void)	/* 1:Successful, 0:Timeout */
{
...
)
{
	...
	spi_init(SPI_PORT, SPI_SPEED_LOW);
	sleep_ms(10);
	for (n = 10; n; n--){
